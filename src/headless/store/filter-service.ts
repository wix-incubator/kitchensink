...
export interface FilterServiceAPI {
  ...
  availableOptions: Signal<{ productOptions: ProductOption[], priceRange: { min: number, max: number }}>;
  calculateAvailableOptions: (products: productsV3.V3Product[]) => Promise<void>;
}

export const FilterServiceDefinition =
  defineService<FilterServiceAPI>("filtered-collection");
...
export const FilterService = implementService.withConfig<{}>()(FilterServiceDefinition, ({ getService }) => {
  ...
  const currentFilters: Signal<Filter> = signalsService.signal(defaultFilter as any);
  ...
  const availableOptions: Signal<{ 
    productOptions: ProductOption[],
    priceRange: { min: number, max: number }
  }> = signalsService.signal({ productOptions: [], priceRange: { min: 0, max: 1000 } } as any);
  ...
  const calculateAvailableOptions = async (products: productsV3.V3Product[]) => {
