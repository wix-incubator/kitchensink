    ...
    seoData: {
      tags: [
        { type: "title", children: "Wix Stores - E-commerce Solutions" },
        {
          ...
          props: {
            name: "description",
            content: "Explore Wix Stores functionality with multiple implementation examples."
          }
        }
      ]
    }
...
```ts
import type { SEOTagsServiceConfig } from "../../headless/seo/services/seo-tags-service";
...
test("updates SEO tags on trigger", async () => {
  ...
  render(
    <SEO.UpdateTagsTrigger seoTagsServiceConfig={config}>
      {({ updateSeoTags }) => (
        <button onClick={() => updateSeoTags(seoTags.ItemType.STORES_PRODUCT, { slug: "test" })}>
          Update
        </button>
      )}
    </SEO.UpdateTagsTrigger>
